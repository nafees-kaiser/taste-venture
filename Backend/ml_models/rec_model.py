...
csv_res = os.path.join(os.path.dirname(__file__), 'final_final_res.csv')
csv_rating = os.path.join(os.path.dirname(__file__), 'final_final_ratings.csv')
...
rating_final = pd.read_csv(csv_rating)
# import os
#
...
def calculate_feature_matrix():
    restaurant_features = ColumnTransformer(
        transformers=[
            ('cuisine', OneHotEncoder(),
             ['cuisine_American', 'cuisine_Armenian', 'cuisine_Bakery', 'cuisine_Bar', 'cuisine_Bar_Pub_Brewery',
              'cuisine_Breakfast-Brunch', 'cuisine_Burgers', 'cuisine_Cafe-Coffee_Shop',
              'cuisine_Cafeteria', 'cuisine_Chinese', 'cuisine_Contemporary', 'cuisine_Family', 'cuisine_Fast_Food',
              'cuisine_Game', 'cuisine_International',
              'cuisine_Italian', 'cuisine_Japanese', 'cuisine_Mexican', 'cuisine_Pizzeria', 'cuisine_Regional',
              'cuisine_Seafood', 'cuisine_Vietnamese']),
            ('payment', OneHotEncoder(),
             ['payment_American_Express', 'payment_Carte_Blanche', 'payment_MasterCard-Eurocard', 'payment_VISA',
    ...
    return restaurant_feature_matrix
...
def create_user_profile(user_id, ratings_df, restaurants_df, feature_matrix, min_rating=1):
    ...
    rated_restaurant_indices = restaurants_df[
        restaurants_df['placeID'].isin(rated_restaurant_ids)].index.values.tolist()
...
class RecModel:
    def __init__(self):
        user_profiles = {}
        self.restaurant_features = calculate_feature_matrix()
        for user_id in rating_final['userID'].unique():
            user_profile = create_user_profile(user_id, rating_final, df_grouped, self.restaurant_features)

            # Store the user profile in the dictionary
            user_profiles[user_id] = user_profile

        self.user_profiles_df = pd.DataFrame(user_profiles).T

    # user_profiles_df.head()

    def calculate_similarity(self, user_vector, restaurant_features):
        return cosine_similarity(user_vector, restaurant_features)

    def recommend(self, userid, top_n=20):
        user = self.user_profiles_df.loc[userid].values.reshape(1, -1)
        similarity = self.calculate_similarity(user, self.restaurant_features)
